export const compiler = function (input) {
    ...
    const addLet = RegExp(`(?<!(?:const|public|private|protected) )(?:int|str|bool|double|big|obj|Array<.*>|Collection<.*>${types}|\\w+\\[])(\\w*?\\[])?\\s+?\\w+\\s*[=;]`,'g');
    const regex = RegExp(`\\b(int|str|bool|double|big|obj|Array<.*>|Collection<.*>${types}|\\w+\\[])(\\w*?\\[])?\\s+(\\w+\\??)`,'g');
